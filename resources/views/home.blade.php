			...
			<div class="row">
				<div class="col-12 col-lg-7">
					...
					<div class="hero__content hero__content--first">
						<h1 class="hero__title"><strong>{{ $settings->title }}</strong></h1><h3 class="text-white"><br>Next-generation cloud mining <br>platform. 
						secure, scalable, and simple <br>access to cryptocurrency mining.</h3>
           <p class="text-white"> At {{ $settings->title }}, we believe in a decentralized future where anyone, anywhere can earn from cryptocurrency mining without the need for expensive hardware, complicated setups, or technical know-how.
</p>
<!--<img src="{{ asset('assets/img/ban1.png') }}" alt="Miner" class="img-fluid" style="max-width: 300px;">-->
						<div class="hero__btns">
							<a href="{{ route('register') }}" class="hero__btn">Sign Up</a>
							<a href="{{ route('about') }}" class="hero__btn hero__btn--light">About us</a>
						</div>
...
<span class="stats__value">{{ $daysSinceLaunch }}</strong> {{ $daysSinceLaunch == 1 ? '' : '' }}</span>
...
<table class="deals__table">
    ...
    <tbody>
        @foreach ($plans as $plan)
            ...
            @endphp
            <tr>
                <td><img src="{{ asset('assets/img/miner.gif') }}" alt="Miner" class="img-fluid" style="max-width: 40px;">
</td>
                <td><div class="deals__text deals__text--buy"><b>{{ $plan->name }}</b></div></td>
                <td class=" deals__text deals__text--buy">{{ $plan->duration }} day{{ $plan->duration > 1 ? 's' : '' }}</td>
                <td><div class="deals__text deals__text--buy">{{ $asset }}</div></td>
                <td>
                    @if($rate)
                      <span class=" deals__text deals__text--buy fiat-value">${{ number_format($priceFiat, 2) }}</span>
                        <span class="deals__text deals__text--buy crypto-value d-none">≈ {{ number_format($priceCrypto, 6) }} {{ $asset }}</span>
                    @else
                        <span style="color: #aaa;">Price Unavailable</span>
                    @endif
                </td>
                <td><div class="deals__text node__title--orange"><b>{{ $plan->hashrate }}</b></div></td> <!-- New Hashrate -->
            <td><div class="deals__text">{{ $plan->power_charge }}</div></td> <!-- New Power Charge -->

                <td ><div class="deals__text node__title--blue"><b>{{ ucfirst($plan->roi_value) }}%</b></div></td>
                <td><div class="deals__text deals__text--buy">{{ ucfirst($plan->roi_type) }}</div></td>
                <td>
                    @if($rate)
                        <div class="deals__text node__title--green"><b><span class="fiat-value">${{ number_format($profitFiat, 2) }}</span></b></div>
                        <div class="deals__text deals__text--green"><span class="crypto-value d-none">{{ number_format($profitCrypto, 6) }} {{ $asset }}</span></div>
                    @else
                        <span style="color: #aaa;">Unavailable</span>
                    @endif
                </td>
                <td><div class="deals__text deals__text--buy">{{ $plan->badge ?? 'N/A' }}</div></td>
            </tr>
        @endforeach
    </tbody>
</table>
				...
				<div class="col-12 col-lg-6">
                    ...
                    <p class="text-white"></p>
						<ul class="invest__list">
							<li><img src="{{ asset('assets/img/iceland.png') }}" alt="Miner" class="img-fluid" style="max-width: 20px;"> <b>Iceland</b> (cooling efficiency)</li>
							<li><img src="{{ asset('assets/img/united-states-of-america.png') }}" alt="Miner" class="img-fluid" style="max-width: 20px;"> <b>USA</b>  (secure & scalable)</li>
							<li><img src="{{ asset('assets/img/united-arab-emirates.png') }}" alt="Miner" class="img-fluid" style="max-width: 20px;"><b> UAE</b> (solar-powered operation)</li>
							<li><img src="{{ asset('assets/img/singapore (1).png') }}" alt="Miner" class="img-fluid" style="max-width: 20px;"><b> Singapore</b> (Fast fiber-optic backbone)</li>
						</ul>
					...
					<div class="invest">
						...
						<div class="invest__rate-wrap">
							...
							<div class="invest__graph">
								<img src="{{ asset('assets/img/graph/graph2.svg') }}" alt="">
							</div>
						</div>
						...
						<div class="invest__rate-wrap">
							...
							<div class="invest__graph">
								<img src="{{ asset('assets/img/graph/graph1.svg') }}" alt="">
							</div>
						</div>
									...
									<div class="accordion" id="accordion">
										<div class="row">
											<div class="col-12 col-lg-6">
												...
												<div class="accordion__card">
													<button class="collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse2" aria-expanded="false" aria-controls="collapse2">How does {{ $settings->title }} cloud mining work?</button>
