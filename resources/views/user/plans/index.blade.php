			...
			<div class="row">
				...
				<div class="col-12 col-md-10 offset-md-1 col-lg-6 offset-lg-3">
					<div class="section__title">
						<h1>{{ $settings->title }} Mining Racks</h1>
	...
	<div class="section section--pb">
		<div class="container">
			<div class="row row--relative">
				<div class="col-12">
					<div class="about">
						<h2 class="about__title">{{ $settings->title }} Mining Plans</h2>

						<p class="about__text">{{ $settings->title }} mining plans are strategically designed to offer investors reliable 
    ...
    <div class="col-12 col-md-6 col-lg-4">
        ...
        <div class="node {{ $plan->sold_out ? 'opacity-50' : '' }}">
            ...
            <p class="node__title--green">
                ...
                <b>{{ $plan->asset->name ?? $plan->asset_id }}</b>
            </p>
            ...
            <ul class="node__list">
                ...
                <li><i class="ti ti-circle-check"></i><b>${{ number_format($plan->expected_profit, 2) }}</b> Expected Profit</li>
                <li><i class="ti ti-circle-check"></i><b>{{ $plan->badge ?? 'N/A' }}</b></li>
            </ul><br>

            <button type="button"
                class="text-white confirm-btn apool__btn {{ $plan->sold_out ? 'disabled' : '' }}"
                data-id="{{ $plan->id }}"
                data-name="{{ $plan->name }}"
                {{ $plan->sold_out ? 'disabled style=background-color:gray;cursor:not-allowed;' : '' }}>
                {{ $plan->sold_out ? 'SOLD OUT' : 'MINE CRYPTO' }}
            </button>
